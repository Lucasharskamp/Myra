...
2. Myra's own MML lacks proper IDE tooling and requires engineers to learn new things; and if one already knows XAML (from Windows.Forms, WPF, Xamarin/MAUI, AvaloniaUI, etc.) it makes the transition a lot easier, which benefits small game studios. It also provides the default XAML features which MML cannot provide.
...
Samples are located in the _/samples/_ folder.
